...
/**
 * Google sign-in is enabled again — the owner's decision on 2026-09-17 to bring the
 * button back on the site and in the app at the same time. The provider was never
 * removed from the Firebase project: accounts that were created with Google still
 * exist and still own their projects, and `assets/app.js` still re-authenticates
 * such an account with a Google popup when it asks to be deleted, because that is
