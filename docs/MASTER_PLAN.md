...
| 3 | **Opis w sklepie wysyła ludzi na martwą domenę** | Play Console → Główna karta sklepu (**11 języków**) i „Co nowego" | **Zmierzone ponownie 31.08.2026**, dziesięć żywych stron sklepu: `materio-app.com` występuje **dwa razy w każdym języku, ale w DWÓCH RÓŻNYCH POLACH**. (1) **Pełny opis**, sekcja „KONTO I STRONA": „Wszystkie kalkulatory działają też na stronie materio-app.com". (2) **Co nowego**, w notatkach 1.10.2: „użyj »Nie pamiętam hasła« na materio-app.com". Host odpowiada **404**, `liczmat.com` odpowiada **200** (oba sprawdzone tego dnia) | Drugie zdanie kieruje kogoś, kto stracił dostęp do konta, pod adres, którego nie ma. **Naprawa nie wymaga AAB — to samo pole tekstowe.** Pole (2) naprawia się przy okazji wklejenia notatek 1.11.0 z `docs/RELEASE_NOTES_1.11.0.md`, które żadnej domeny nie wymieniają. Pole (1) to jedna podmiana `materio-app.com` → `liczmat.com` na język |
