...
function quoChooseClient(clientId) {
  ...
  let job = crmJobOfProject(project.id);
  if (job) job = crmUpdateJob(job.id, { clientId: client.id });
  else job = crmAddJob({ name: q.name, clientId: client.id, projectId: project.id });
  if (!job) return null;
  crmLinkProject(client.id, project.id);
  return crmUpdateQuote(q.id, { projectId: project.id });
}

/** A job without a project gains one, because the quote can only point at that project. */
function quoChooseJob(jobId) {
  const q = crmQuote(quoOpenId);
  let job = crmJob(jobId);
  if (!q || !job) return null;
  let project = job.projectId && wsProject(job.projectId);
  if (!project) {
    project = wsAddProject(job.name);
    if (!project) return null;
    job = crmUpdateJob(job.id, { projectId: project.id });
  }
  if (job.clientId) crmLinkProject(job.clientId, project.id);
  return crmUpdateQuote(q.id, { projectId: project.id });
}
...
function quoRenderProject(q) {
  ...
  quoFillPicker("quo-job-pick", crmAllJobs(), chain.job && chain.job.id, "crm_node_none");
...
function wireQuoteDetail() {
  ...
  on("quo-job-form", "submit", (e) => {
    e.preventDefault();
    const pick = document.getElementById("quo-job-pick");
    if (pick && pick.value) quoChooseJob(pick.value);
  });
  ...
  on("quo-job-new-form", "submit", (e) => {
    e.preventDefault();
    const input = document.getElementById("quo-job-new");
    const q = crmQuote(quoOpenId);
    if (!input || !q || !input.value.trim()) return;
    const chain = crmChain("quote", q.id);
    const project = wsAddProject(input.value);
    const row = project && crmAddJob({ name: input.value, projectId: project.id,
      clientId: chain.client ? chain.client.id : "" });
    if (row) { input.value = ""; quoChooseJob(row.id); }
  });
  ...
  on("quo-project-new-form", "submit", (e) => {
    ...
    if (chain.job) crmUpdateJob(chain.job.id, { projectId: project.id });
