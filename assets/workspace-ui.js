...
let wsAsking = false;
/** Project row whose inline delete question is open, or "" when none is. */
let wsAskingRowId = "";
...
function wsProjectRow(p) {
  ...
  return `<li class="ws-proj-row" data-id="${wsEsc(p.id)}">
      <span class="row-name">
      ...
      <span class="row-actions">
        ${p.archived
          ...
          : ""}
        <button type="button" class="btn btn-ghost btn-sm" data-del-project>${wsEsc(wsT("proj_delete"))}</button>
      </span>
      ${wsAskingRowId === p.id ? wsDeleteAsk(p) : ""}
    </li>`;
}

/** Build the warning once so the index and detail never disagree about what is deleted. */
function wsDeleteQuestion(project) {
  const lines = wsProjectTotal(project.id).count;
...
function wsDeleteAsk(project) {
  return `<div class="ws-ask ws-row-ask">
      <p>${wsEsc(wsDeleteQuestion(project))}</p>
      <p class="muted">${wsEsc(wsT("ws_delete_undo_hint"))}</p>
      <p class="ws-ask-row">
        <button type="button" class="btn btn-primary btn-sm" data-confirm-del-project>${wsEsc(wsT("proj_delete_yes"))}</button>
        <button type="button" class="btn btn-ghost btn-sm" data-cancel-del-project>${wsEsc(wsT("app_cancel"))}</button>
      </p>
    </div>`;
}
...
function wsRenderProject(id) {
  ...
  ask.hidden = !wsAsking;
  document.getElementById("ws-delete-q").textContent = wsDeleteQuestion(project);
 ...
 * `projectId` to send.
 */
function wsProjectOptions(current, allowEmpty = true) {
  return (allowEmpty ? `<option value="">${wsEsc(wsT("ws_room_free"))}</option>` : "") + wsProjects()
...
function wsRenderRooms() {
  ...
  const rooms = wsRooms();
  if (!projects.length) {
    list.innerHTML = `<p class="muted">${wsEsc(wsT("ws_rooms_need_project"))}</p>`;
    return;
  }
  ...
  const card = (project, groupedRooms, projectIndex) => {
    const projectId = project.id;
    const storedColor = roomColors.indexOf(project.color) >= 0 ? project.color : "";
    const color = storedColor || roomColors[projectIndex % roomColors.length];
    const title = `<a href="?id=${encodeURIComponent(project.id)}">${wsEsc(project.name)}</a>`;
...
function buildProjectsPage() {
  ...
  const rowAction = (e) => {
    ...
    else if (e.target.closest("[data-del-project]")) {
      wsAskingRowId = li.dataset.id;
      wsRenderProjects();
      const row = [...document.querySelectorAll("li[data-id]")]
        .find((node) => node.dataset.id === wsAskingRowId);
      const cancel = row && row.querySelector("[data-cancel-del-project]");
      if (cancel) cancel.focus();
    } else if (e.target.closest("[data-cancel-del-project]")) {
      wsAskingRowId = "";
      wsRenderProjects();
    } else if (e.target.closest("[data-confirm-del-project]")) {
      const project = wsProject(li.dataset.id);
      if (!project) return;
