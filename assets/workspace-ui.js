...
function wsRenderRooms() {
  ...
  const card = (project, groupedRooms, projectIndex) => {
    ...
    const storedColor = project && roomColors.indexOf(project.color) >= 0 ? project.color : "";
    const color = project ? (storedColor || roomColors[projectIndex % roomColors.length]) : "";
    const title = project
  ...
  list.innerHTML = projects.map((p, i) => card(p, rooms.filter((r) => r.projectId === p.id), i)).join("")
    + (loose.length || !projects.length ? card(null, loose, -1) : "");
