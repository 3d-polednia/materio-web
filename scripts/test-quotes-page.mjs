...
head("7. chapter XXV's paywall: the wall, the two rungs and the one door through it");
{
  ...
  const guest = await open(ctx, QUOTES, { workspace: workspace(), crm: crm(), preview: false });
  ...
  const free = await open(ctx, QUOTES, { workspace: workspace(), crm: crm(), level: "liczmat", preview: false });
  ...
  await free.click('#quo-gate [data-pw-preview]');
  await free.waitForSelector("#quo-tool:not([hidden])");
  eq("the wall comes down", await free.$eval("#quo-gate", (n) => n.hidden), true);
  eq("the strip comes back", await free.$eval("#quo-pro", (n) => n.hidden), false);
  check("saying a preview is running, not a plan",
    (await free.textContent("#quo-pro-chip")) === (await free.evaluate(() => t("pro_prev_chip"))),
    await free.textContent("#quo-pro-chip"));
  eq("with the sentence that says the account did not change",
    await free.$eval("#quo-pro-note", (n) => n.hidden), false);
  eq("the chip is not the one a paying account gets",
    await free.$eval("#quo-pro-chip", (n) => n.classList.contains("on")), false);
  ...
  await free.click('#quo-pro [data-pw-preview]');
  await free.waitForSelector("#quo-gate:not([hidden])");
  eq("turning the preview off restores the wall",
    await free.$eval("#quo-tool", (n) => n.hidden), true);
  ...
  const pro = await open(ctx, QUOTES, { workspace: workspace(), crm: crm(), level: "pro", preview: false });
  ...
  eq("nothing lectures them", await pro.$eval("#quo-pro-note", (n) => n.hidden), true);
  eq("and there is no preview to turn off",
    await pro.$$eval("#quo-pro [data-pw-preview]", (n) => n.every((b) => b.hidden)), true);
  await pro.close();
}
