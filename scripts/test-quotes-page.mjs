...
head("7. chapter XXV: the module names itself Pro to everybody");
{
  const guest = await open(ctx, QUOTES, { workspace: workspace(), crm: crm() });
  check("a guest is told what this is", (await guest.textContent("#quo-pro-chip")).length > 0);
  eq("and the module still runs — nothing grants Pro yet",
    await guest.$eval("#quo-tool", (n) => n.hidden), false);
  eq("so the gate stays out of the way", await guest.$eval("#quo-gate", (n) => n.hidden), true);
  check("with the honest sentence under it",
    (await guest.$eval("#quo-pro-note", (n) => n.hidden)) === false);
  await guest.close();

  const pro = await open(ctx, QUOTES, { workspace: workspace(), crm: crm(), level: "pro" });
  eq("a Pro account is not lectured", await pro.$eval("#quo-pro-note", (n) => n.hidden), true);
  eq("and the module runs for them too", await pro.$eval("#quo-tool", (n) => n.hidden), false);
  await pro.close();
}
