...
head("7. chapter XXV's paywall: the wall, the two rungs and the one door through it");
{
  ...
  eq("the wall no longer says the subscription is waiting",
    await free.$eval("#quo-gate [data-pw-soon]", (n) => n.hidden), true);
  eq("and names the way in instead",
    await free.$eval("#quo-gate [data-pw-buy]", (n) => n.hidden), false);
  check("which is /app/, the one page that knows whose payment it is",
    (await free.$eval("#quo-gate [data-pw-buy] a", (n) => n.getAttribute("href") || "")).includes("/app/"));
