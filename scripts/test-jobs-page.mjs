...
head("6. chapter XXV's paywall: the wall, the two rungs and the one door through it");
{
  ...
  const guest = await open(ctx, JOBS, { workspace: workspace(), crm: crm(), pro: false });
  ...
  const free = await open(ctx, JOBS, { workspace: workspace(), crm: crm(), level: "liczmat", pro: false });
  ...
  eq("the monthly plan is priced", await free.$eval('#job-gate [data-pw-plan="monthly"]', (n) => n.hidden), false);
  eq("and the yearly one", await free.$eval('#job-gate [data-pw-plan="yearly"]', (n) => n.hidden), false);
  check("with a real amount in it",
    /[0-9]/.test(await free.$eval('#job-gate [data-pw-plan="monthly"] [data-pw-price]', (n) => n.textContent)),
    await free.$eval('#job-gate [data-pw-plan="monthly"] [data-pw-price]', (n) => n.textContent));
  eq("the wall no longer says the subscription is waiting",
    await free.$eval("#job-gate [data-pw-soon]", (n) => n.hidden), true);
  eq("and names the way in instead",
    await free.$eval("#job-gate [data-pw-buy]", (n) => n.hidden), false);
  check("which is /app/, the one page that knows whose payment it is",
    (await free.$eval("#job-gate [data-pw-buy] a", (n) => n.getAttribute("href") || "")).includes("/app/"));
  ...
  eq("the wall is still standing", await free.$eval("#job-gate", (n) => n.hidden), false);
  eq("and the module is still behind it", await free.$eval("#job-tool", (n) => n.hidden), true);
  await free.close();

  /* A Pro account walks straight in, and is told which plan opened it. */
  const pro = await open(ctx, JOBS, { workspace: workspace(), crm: crm(), level: "pro", pro: false });
  ...
  eq("and is not quoted a price for what they already pay for",
    await pro.locator('#job-gate [data-pw-plan="monthly"]').isVisible(), false);
  await pro.close();
}
