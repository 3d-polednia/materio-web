...
head("9b. the LiczMat Pro tab: what the plan is, and no way to buy one");
{
  ...
  // Chapter XXV's rule is "never a dead button", not "never a button". Sessions 22 and 23
  // built Klienci and Zlecenia, so their cards are the only things in the panel that open
  // anything; the other three modules do not exist yet and stay text, and there is still
  // nothing to buy, because no payment exists (FIRESTORE_SYNC §9.2).
  eq("the only controls in the panel are the modules that have been built",
    await free.locator("#panel-pro a, #panel-pro button").count(), 2);
  eq("the first opens Klienci",
    await free.locator('.pro-mod[data-feature="clients"] a').getAttribute("href"), "/klienci/");
  eq("and the second Zlecenia",
