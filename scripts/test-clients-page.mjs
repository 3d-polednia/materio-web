...
head("5. chapter XXV's paywall: the wall, the two rungs and the one door through it");
{
  ...
  const free = await open(ctx, CLIENTS, { workspace: workspace(), clients: clients(), level: "liczmat", pro: false });
  ...
  eq("the wall is still standing", await free.$eval("#crm-gate", (n) => n.hidden), false);
  eq("and the module is still behind it", await free.$eval("#crm-tool", (n) => n.hidden), true);
  await free.close();

  /* A Pro account walks straight in, and is told which plan opened it. */
  const pro = await open(ctx, CLIENTS, { workspace: workspace(), clients: clients(), level: "pro", pro: false });
  ...
  eq("and is not quoted a price for what they already pay for",
    await pro.locator('#crm-gate [data-pw-plan="monthly"]').isVisible(), false);
  await pro.close();
}
