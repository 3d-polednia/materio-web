...
function writeTranslationsTodo() {
  ...
  const out = [
    ...
    "languages frozen at the last full build. Every string below exists in Polish and is",
    "missing in the languages its own row names — usually all twelve, but read the row, not",
    "the count. Nothing on this list is live anywhere: a frozen page cannot show a key that",
...
// Before the abort, not after it. `pending` is complete the moment validate() returns, and
// a build that stops for an unrelated reason must not leave a ledger describing the run
// before last — a stale worklist is worse than none, because it reads like a current one.
